...
const View = () => {
    ...
    const [htmlVal, sethtmlVal] = useState(`${project?.htmlVal}`);
    const [cssVal, setcssVal] = useState(`${project?.cssVal}`);
    const [jsVal, setjsVal] = useState(`${project?.jsVal}`);
    const [finalSrcCodeOutput, setfinalSrcCodeOutput] = useState(`${project?.finalSrcCodeOutput}`);
                ...
                <div className='flex items-center justify-center gap-6'>
                    ...
                    <div className='flex flex-col items-start justify-start '>
                        <div className='flex items-center justify-center gap-3'>
                            ...
                            <p key={"titlab"} className='px-3 py-2 text-white text-lg'>
                                {project?.title}
                            </p>
                        ...
                        <div className='flex items-center justify-center px-3 gap-2 -mt-2 text-[#868CA0]'>
                            <p>By <span className='text-emerald-400 italic'>{project?.user?.displayName ? user?.displayName : project?.user?.email.split("@")[0]}</span></p>
                        </div>
                    </div>
                </div>
                {user && <>
